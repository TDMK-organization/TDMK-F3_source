...
namespace Export_FPCA_OK2ship_Auto_System.Services
{
    ...
    public class AirBubbleService
    {
        ...
        private void SolveFolderPSALINER(string location, string maker, string pai)
        {
            ...
            {
                int num = TDMK_ConverterService.getDigit(s.Split('\\')[s.Split('\\').Count() - 1]);
        ...
        private void solveData(string maker, string item, DataTable dataTable)
        {
            ...
            foreach (KeyValuePair<Image, string> itemZ in listImage)
            {
                try
                {
                    int psc = TDMK_ConverterService.getDigit(itemZ.Value);
            ...
            foreach (string file in listFile)
            {
                try
                {
                    ...
                    int psc = TDMK_ConverterService.getDigit(file.Split('\\')[file.Split('\\').Count() - 1].Trim());
        ...
        private DataTable getDataInBF(string location)
        {
            ...
            foreach (KeyValuePair<Image, string> item in list)
            {
                try
                {
                    ...
                    int psc = TDMK_ConverterService.getDigit(item.Value);
        ...
        public KeyValuePair<string, string> getItemCodeLotNo(string folderName)
        {
            ...
            if (s.Length > 5)
            {
                ...
                try
                {
                    lotNo = TDMK_ValidateService.lotNoHandle($"{s[2]}-{TDMK_ValidateService.isDigit(s[3])}");
                }
                catch
                {
                    lotNo = TDMK_ValidateService.lotNoHandle($"{s[2]}");
                }
        ...
        private void ExportLinerSheet(ExcelWorksheet ws, string refer)
        {
            ...
            IDictionary<string, string> dic = ExportProcess.FindAddressByText(ws, finder.Concat(new[] { "Air bubble", "area:" }).ToArray(), false);
                ...
                if (dic.TryGetValue(keyX, out string address))
                {
                    ...
                    for (int iz = 0; iz < 32; iz++)
                    {
                        ...
                        if (iz < dataTable.Rows.Count)
                        {
                            ...
                            addressSample = ExportProcess.AddRow(addressSample, 1);
                            ws.Cells[addressSample].Value = dataTable.Rows[iz]["Measure(mm)"];
                            addressSample = ExportProcess.AddRow(addressSample, 1);
        ...
        public void Export(ExcelWorksheet ws, string type)
        {
            if (type == "Liner")
            {

                ExportSetupLiner(ws, type);
                //Export liner
                ExportLinerSheet(ws, type);
            }
            if (type == "PSA")
            {

                ExportSetupLiner(ws, type);
                ExportPSASheet(ws, type);
            }
        }
        ...
        {
            IDictionary<string, string> dic = ExportProcess.FindAddressByText(ws, new[] { "supplier", "Air bubble", "area:" }, false);
        ...
        private void ExportSetupLiner(ExcelWorksheet ws, string type)
        {
            ...
            IDictionary<string, string> dicZ = ExportProcess.FindAddressByText(ws, new[] { "Average", "Flex SN", "Air bubble", "supplier", "Sample 32" }, false);

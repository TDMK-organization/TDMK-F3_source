...
namespace Export_FPCA_OK2ship_Auto_System.Services
{
    public class ACFService
    {
        ...
        public string packagingData(Dictionary<string, DataTable> dic, Dictionary<string, List<DateTime>> dic_list)
        {
            ...
            {
                string dataTable = TDMK_ConverterService.DataTableToJson(dic[machine]);
        ...
        public KeyValuePair<Dictionary<string, DataTable>, Dictionary<string, List<DateTime>>> LoadWCA(string itemCode, string lotNo, string type)
        {
            ...
            foreach (string line in dataLines)
            {
                ...
                DataTable dt = TDMK_ConverterService.JsonToDataTable(parts[2]);
        ...
        private Dictionary<string, string> GetMerge(List<string> lists)
        {
            ...
            foreach (string item in lists)
            {
                ...
                {
                    //get new point
                    point = TDMK_ConverterService.getDigit(item).ToString();
                }
        ...
        public void ExportBoding(ExcelWorksheet ws, string itemCode, string lotNo)
        {
            ...
            IDictionary<string, string> dic = TDMK_DictionaryService.MergeDictionaries(ExportProcess.FindAddressByText(ws, list.ToArray(), true), ExportProcess.FindAddressByText(ws, listZ));

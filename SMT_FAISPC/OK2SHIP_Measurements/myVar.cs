...
namespace OK2SHIP_Software
{
    public class myVar
    {
        ...
        public void Export_FAI_Data2(myExcel.Workbook src_format_wrkbook, string tar_ItemCode, string tar_LotNo, string format_type)
        {
            myExcel.Application xlsApp = TDMK_Code.StartExcel();
            xlsApp.Visible = true;
            myExcel.Workbook exp_file;
            exp_file = xlsApp.Workbooks.Add();
            Export_FAI_Batch(tar_ItemCode, tar_LotNo, exp_file, format_type);
    ...
    resume_lable: if (mywrksheet.ProtectContents)
        ...
        public void Export_FAI_Batch(string tar_ItemCode, string tar_LotNo, myExcel.Workbook tar_wrkbook,string format_type, int qty=32)
        {
            char[] trim_char = new char[] { ' ', '\r', '\n' };
            DataTable FAI_Data_tbl = TDMK_Code.Datatable_Filter(myVar.sqlcon_SMT, "FAI_Auto", TDMK_Code.filter_str(new string[] { "ItemCode", "LotNo","Remark" }, new string[] { tar_ItemCode, tar_LotNo, format_type }));

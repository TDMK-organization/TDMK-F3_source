...
namespace OK2SHIP_Measurements
{
    public class SEI_Lib
    {
        ...
        public struct IPQC_Spec
        {
            public string IPQC_Name;
            public string SetVal;
            public string UL;
            public string LL;
            public IPQC_Spec(string _IPQC_Name, string _SetVal, string _UL, string _LL)
            {
                IPQC_Name = _IPQC_Name;
                SetVal = _SetVal;
                UL = _UL;
                LL = _LL;
            }
        ...
        public void Load_Test_Data(string format_loc, string tar_ItemCode, string tar_LotNo, string file_extension, DataGridView tar_DGV_Data)
        {
            ...
            if (File.Exists(tar_format_file))
            {
                ...
                MessageBox.Show("Finished!", "Thong bao");
                //this.Activate();
                //tar_wkbook.Close();
        ...
        public void Export_Data_Process(string app_path, DataGridView tar_DGV, string tar_ItemCode, string tar_LotNo)
        {
            ...
            MessageBox.Show("Hoàn thành xuất dữ liệu", "Thông báo");
        }
        ...
        public void Load_FAI_DGV2(DataGridView tar_DGV, SqlConnection tar_sqlcon, string tar_tbl, string FAI_No_name, string FAI_Data_Col_name, string tar_ItemCode, string tar_LotNo)
        {
            ...
            {
                t.HeaderText = t.HeaderText.Split('_')[0];
                foreach (DataGridViewRow dgv_r in tar_DGV.Rows)
                {
            ...
            foreach(DataGridViewRow dgv_r in tar_DGV.Rows)
            {
                dgv_r.HeaderCell.Value = (r_inx + 1).ToString();
                r_inx++;
            }
            tar_DGV.AutoResizeRowHeadersWidth(DataGridViewRowHeadersWidthSizeMode.AutoSizeToAllHeaders);
            Disable_Sort_DGV(tar_DGV);
        ...
        public bool Load_Spec(SqlConnection sqlcon, string format_loc, string tar_ItemCode, string tar_LotNo, string file_extension, DataGridView tar_DGV_Spec, string tar_Dev)
        {
            ...
            if (FAI_SheetNo_list.Count == 0)
            {
                ...
                //    foreach (myExcel.Worksheet tg in tar_wkbook.Worksheets)
                //    {
                //        if ((tg.Name.Contains("FAI"))|| (tg.Name.Contains("CPK"))|| (tg.Name.Contains("SPC")))
                //        {
                //            myExcel.Range sel_rgn = tg.Range["C14"];
                //            myExcel.Range dev_rgn = tg.Range["C18"];
                //            int sel_inx = 0;
                //            while (checkDBNull(sel_rgn.Offset[0, sel_inx].Value) != "")
                //            {
                ...
                //                string t_FAI_UL = checkDBNull(sel_rgn.Offset[2, sel_inx].Value);
                //                string t_FAI_LL = checkDBNull(sel_rgn.Offset[3, sel_inx].Value);
                //                string dgv_col_name = t_FAIName + "_" + t_FAI_Setval;
                //                string t_FAI_sheetno = tg.Name;
                //                string t_instrument = checkDBNull(dev_rgn.Offset[0, sel_inx].Value);
                //                testFAI_spec.Add(new SEI_Lib.FAI_Spec(dgv_col_name, t_checkside, t_FAI_Setval, t_FAI_UL, t_FAI_LL, t_FAI_sheetno, t_instrument));
                //                if (!check_columns_existed(DGV_To_Table(tar_DGV_Spec), dgv_col_name))
                //                {
                ...
                //    _result = true;
                //    tar_wkbook.Close();
                //    save_FAI_spec(testFAI_spec.ToArray(),tar_ItemCode,sqlcon);
                //}
                //else
                //{
                //    MessageBox.Show("File format khong tim thay");
                //    _result = false;
                //}
                MessageBox.Show(new Form { TopMost = true }, "Spec not found. Please, setup again", "Warning");
            }
            else
            {
 ...
 start_lbl: DataTable Spec_tbl = TDMK_Code.Datatable_Filter(tar_sqlcon, spec_tbl_name, TDMK_Code.filter_str(new string[] { "ItemCode", "Item", "Remark" }, new string[] { tar_ItemCode, "%" + tar_itemcheck + "%", "%AU_Plating%" })); //Roughness AU_NI            
            if (Spec_tbl.Rows.Count == 0)
            {
                ...
                if (file_format_lst.Length > 0)
                {
                    ...
                    {
                        _result = false;
                        MessageBox.Show("File Format not found and no data of ItemCode "+ tar_ItemCode, "Warning");
                    }
                }
                else
                {
                    _result = false;
                    MessageBox.Show("No Spec data in database");
                }                                 
            }
            else
            {
        ...
        public void save_FAI_spec(SEI_Lib.FAI_Spec[] src_FAI_spec, string tar_ItemCode, SqlConnection sqlcon, string type="NPI")
        {
            ...
            foreach (DataColumn tbl_col in test_tbl.Columns)
            {
                ...
                foreach (SEI_Lib.FAI_Spec ref_spec in src_FAI_Spec)
                {
                    if (ref_spec.FAI_Name == t)
                    {
                        ...
                        if (inx>0)
                        {
                            Calcul_CPK_FAI2(ref_spec, data_arr, ref myCalc_CPK[col_list_inx], ref myHistogram_data[col_list_inx]);
                        }                        
                        break;
                    }
                }
        ...
        public bool check_in_limit_old(string src_UL, string src_LL, string src_act_val)
        {
            bool _result = false;
        ...
        {
            Color tar_color = Color.GreenYellow;
            if (IsNumeric(src_act_val))
            {
                if (src_UL != "")
                {
        ...
        public void AU_IN_Scan(string src_file, DataGridView tar_DGV_Data, DataGridView tar_DGV_Spec, DataGridView tar_DGV_Data_Plus, bool _editMode, int qty)
        {
            if (tar_DGV_Spec.RowCount > 0)
            {
                ...
                for (int i = 0; i < qty; i++)
                {
                    ...
                    {
                        if (checkDBNull(sel_DGV.Rows[i].Cells[j].Value) != "")
                        {
                            double UL = Convert.ToDouble(tar_DGV_Spec.Rows[1].Cells[j].Value);
                            double LL = Convert.ToDouble(tar_DGV_Spec.Rows[2].Cells[j].Value);
                            double act_val = Convert.ToDouble(sel_DGV.Rows[i].Cells[j].Value);
                            if (act_val > UL)
                            {
                            ...
                            if (act_val < LL)
                            {
                                sel_DGV.Rows[i].Cells[j].Style.BackColor = Color.LightBlue;
        ...
        public bool insert_FAI_data_DGV(DataGridView src_DGV, DataGridView tar_DGV, DataTable src_tbl_spec)
        {
            ...
            if (check_columns_existed(src_tbl_spec, src_col_name))
            {
                if (!check_columns_existed(DGV_To_Table(tar_DGV), src_col_name))
                {
                    ...
                    double UL = Convert.ToDouble(sv) + Convert.ToDouble(TolMax);
                    double LL = Convert.ToDouble(sv) - Convert.ToDouble(TolMin);
                    if (tar_DGV.Rows.Count > 0)
                    {
                        if (src_DGV.Rows.Count == tar_DGV.Rows.Count)
                        {
                            ...
                            for (int i = 0; i < src_DGV.Rows.Count; i++)
                            {
                                ...
                                if (check_in_limit(UL.ToString(),LL.ToString(),act_val,sv))
                                {
                                    tar_DGV.Rows[i].Cells[src_col_name].Style.BackColor = Color.White;
                                }
                                else
                                {
                                    if (Convert.ToDouble(act_val) > UL)
                                    {
                                        tar_DGV.Rows[i].Cells[src_col_name].Style.BackColor = Color.Yellow;
                                    }
                                    else
                                    {
                                        tar_DGV.Rows[i].Cells[src_col_name].Style.BackColor = Color.LightBlue;
                                    }
                                    //confirm_request = true;                                    
                                }
                            }
                        ...
                        {
                            MessageBox.Show("Data size not suitable!", "Warning");
                            return false;
                        }
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < tar_DGV.RowCount; i++)
                        {
                            ...
                            {
                                //tar_DGV.Rows[i].Cells[src_col_name].Style.BackColor = Color.Red;
                                if (Convert.ToDouble(act_val) > UL)
                                {
                                    tar_DGV.Rows[i].Cells[src_col_name].Style.BackColor = Color.Yellow;
                                }
                ...
                {
                    MessageBox.Show(src_col_name + " is already existed, cannot add more!", "Warning");
                }
            }
            else
            {
                MessageBox.Show("Please, change to suitable items!","Warning");
            }
            return _result;
        }
                        ...
                        if (src_DGV.Rows.Count == tar_DGV.Rows.Count)
                        {
                            ...
                            for (int i = 0; i < src_DGV.Rows.Count; i++)
                            {
                                ...
                                if ((spec_check_en)&&(act_val!=""))
                                {
                                    ...
                                    {
                                        if (Convert.ToDouble(act_val) > Convert.ToDouble( UL))
                                        {
                                            tar_DGV.Rows[i].Cells[src_col_name].Style.BackColor = Color.Yellow;
                                        }
                                        else
                                    ...
                                    try
                                    {
                                        if (checkDBNull(cell.Value) != "")
                                        {
                                            ...
                                            if (check_columns_existed(DGV_To_Table(tar_DGV_Spec), curr_col_name))
                                            {
                                                ...
                                                if (check_in_limit(_UL.ToString(), _LL.ToString(), act_val,SV))
                                                {
        ...
        public void Check_Data_inSpec(DataGridView src_DGV_Data, DataGridView src_DGV_Spec)
        {
            foreach (DataGridViewColumn dgv_col in src_DGV_Data.Columns)
            {
                ...
                double SetVal = Convert.ToDouble(src_DGV_Spec.Rows[0].Cells[col_name].Value);
                string UL = "";
                string LL = "";
                ...
                foreach (DataGridViewRow dgv_row in src_DGV_Data.Rows)
                {
                    ...
                    dgv_row.Cells[col_name].Style.BackColor = check_in_limit5(UL, LL, act_val,SetVal.ToString());
                }
            }
        }
        public void Check_IPQC_Data_inSpec(DataGridView src_DGV_Data, DataGridView src_DGV_Spec)
        {
            foreach (DataGridViewColumn dgv_col in src_DGV_Data.Columns)
            {
                string col_name = dgv_col.Name;
                string UL = checkDBNull(src_DGV_Spec.Rows[1].Cells[col_name].Value);
                string LL = checkDBNull(src_DGV_Spec.Rows[2].Cells[col_name].Value);

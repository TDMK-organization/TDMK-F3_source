...
namespace Bending_Items
{
    public class ECheck_Process
    {
        ...
        public DataTable Tayo_3GMRD_Process(string logfile, int num_NET, ref DataTable tar_NET_spec)
        {
        ...
        public DataTable Yamaha_LogFile(string filename, ref DataTable tbl_Net_Spec)
        {
            ...
            return dt1.AsDataView().ToTable(false, data_col.ToArray());
        }
        ...
        //    return dgv_dt;
        //}
        public DataTable Detail_NET_PCS2(DataTable src_tbl)
        {
            DataTable dgv_dt = new DataTable();
            List<DataTable> myLst_tbl = new List<DataTable>();
            int col_count = src_tbl.Columns.Count;
        ...
        public DataTable Detail_NET_PCS(DataTable src_tbl)
        {
            ...
            {
                if (!myCode.check_columns_existed(dgv_dt, pcs))
                {
                    dgv_dt.Columns.Add(pcs);
                }
                List<string> Net_data_lst = dt.AsEnumerable().Where(x => x.Field<string>("Pcs_No") == pcs).Select(x => x.Field<string>(cycle_name)).ToList();
                for (int r = 0; r < Net_data_lst.Count; r++)
                {
                    if (dgv_dt.Rows.Count <= r)
                    {
                        dgv_dt.Rows.Add();
                    }
                    dgv_dt.Rows[r][pcs] = Net_data_lst[r];
                }
            }
            //Get_ListTable(-1, dt, new string[] { "ItemCode", "LotNo", "Pcs_No" }, ref myLst_tbl, cycle_name);
            //foreach (DataTable pcs_dt in myLst_tbl)
            //{
            //    string pcs = pcs_dt.Rows[0]["Pcs_No"].ToString();
            //    List<string> Net_data_lst = pcs_dt.AsEnumerable().Select(x => x.Field<string>(cycle_name)).ToList();
            //    if (!myCode.check_columns_existed(dgv_dt, pcs))
            //    {
            //        dgv_dt.Columns.Add(pcs);
            //    }
        ...
        public List<NG_list> Get_NG_point2(DataGridView dgv_spec, DataGridView dgv_data)
        {
            ...
            foreach (DataGridViewColumn dc in dgv_data.Columns) //foreach (DataGridViewRow dr in dgv_data.Rows)
            {
                ...
                {
                    string USL = dgv_spec.Rows[dr.Index].Cells["USL"].Value.ToString();
                    string LSL = dgv_spec.Rows[dr.Index].Cells["LSL"].Value.ToString();
        ...
        public List<NG_list2> Get_NG_point_tbl(DataTable dgv_spec, DataTable dgv_data)
        {
            ...
            foreach (DataColumn dc in dgv_data.Columns)
            {
                ...
                {
                    string USL = dgv_spec.Rows[r_inx]["USL"].ToString();
                    string LSL = dgv_spec.Rows[r_inx]["LSL"].ToString();
...
start_lbl: string filter_str = TDMK_Code.filter_str(new string[] { "ItemCode", "Remark" }, new string[] { ItemCode, Process_name });
        ...
        public DataTable Summary_Selected_FromExisted(DataTable src_Sum, List<NG_list2> src_NG, int inQty)
        {
            ...
            if(item_OK>0)
            {
                ...
                if(sel_Qty< inQty)
                {
                    ...
                    {
                        qty = Math.Min( sel_NG_qty, src_Sum.Columns.Count);
                        for (int i = 0; i < qty; i++)
                        {
                            sel_item_lst.Add(src_Sum.Columns[i].ColumnName);
                        }
            ...
            _result = src_Sum.AsDataView().ToTable(false, sel_item_lst.Distinct().ToArray());
            return _result;
        }
        ...
        public DataTable Save_Submit_data(SqlConnection sqlcon_OK2SHIP, DataTable submit_tbl, string ItemCode, string LotNo, string process_name, string cycle_name)
        {
            ...
            else
            {
                ...
                if(save_en)
                {
                    ...
                    {
                        int r_inx = 0;
                        string pcs_no = dc.ColumnName.Split('_').First();
                        foreach (DataRow dr in submit_tbl.Rows)
                        {
                            id++;
        ...
        public DataTable Save_Submit_data(SqlConnection sqlcon_OK2SHIP, DataTable submit_tbl, string ItemCode, string LotNo, string process_name, string cycle_name, string shift)
        {
            start_lbl: string filter_str = TDMK_Code.filter_str(new string[] { "ItemCode", "LotNo" }, new string[] { ItemCode, LotNo });
            DataTable temp_dt = TDMK_Code.Datatable_Filter(sqlcon_OK2SHIP, process_name, filter_str);
            if (cycle_name == "")
            ...
            else
            {
                ...
                if (save_en)
                {
                    ...
                    foreach (DataColumn dc in submit_tbl.Columns)
                    {
                        ...
                        {
                            id++;
                            TDMK_Code.insert_val_arr2(process_name, sqlcon_OK2SHIP, new string[] { "ID", "ItemCode", "LotNo", "Net_No", "Pcs_No", cycle_name, "Shift" }, new string[] { id.ToString(), ItemCode, LotNo, (r_inx + 1).ToString(), pcs_no, dr[dc].ToString(),shift });
                            r_inx++;
                        }
                        col_inx++;
                    }
...
start_lbl:  DataTable src_dt = TDMK_Code.Datatable_Filter(sqlcon_OK2SHIP, process_name, filter_str);
        ...
        public void Electrical_CPK_Details(DataTable src_tbl, DataGridView DGV_Data, SqlConnection sqlcon_OK2SHIP, DataGridView DGV_NG_detail, string ItemCode, string process_name)
        {
            ...
            foreach (List<string> lst in src_NET_data_lst)
            {
                ...
                double USL_data = cur_net_val.Max() * 1.1;
                double LSL_data = cur_net_val.Min() * 0.9;

                double CPK = cur_net_val.ToArray().CPK(USL_data, LSL_data);

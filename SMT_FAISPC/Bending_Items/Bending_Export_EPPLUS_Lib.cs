...
namespace Bending_Export
{
    public class Bending_Export_EPPLUS_Lib
    {
        ...
        public void Export_Thermal_HeatSoak_Bend_All(SqlConnection sqlcon_OK2SHIP, string ItemCode, string LotNo, string process_name, string format_name, int qty, string format_type)
        {
            ...
            if (format_file != "")
            {
                ...
                if (tar_wrksht_name != "")
                {
                    ...
                    string report_name = process_name + " " + itemname + "-" + ItemCode + "-" + LotNo + " " + DateTime.Now.ToString("yyyyMMdd_HHmmss") + Path.GetExtension(format_file_info.Name);
                    if (format_type == "NPI")
                    {
                        report_name = Path.GetFileNameWithoutExtension(format_file_info.Name) + "-" + LotNo + " " + DateTime.Now.ToString("yyyyMMdd_HHmmss") + Path.GetExtension(format_file_info.Name);
                    }
                    string daily_folder = Path.Combine(Report_location, "OK2SHIP_report_internal");
                    if (!Directory.Exists(daily_folder))
                    {
                        Directory.CreateDirectory(daily_folder);
                    }
                    ...
                    while (report_wrk.Worksheets.Count>1)
                    {
                        foreach (ExcelWorksheet sht in report_wrk.Worksheets)
                        {
                            if (sht.Name != tar_wrksht_name)
                            {
                                report_wrk.Worksheets.Delete(sht);
                            }
                        }
                    }
                    tar_wrksht = report_wrk.Worksheets[tar_wrksht_name];
                    ...
                    for (int col_inx = 0; col_inx < sel_qty; col_inx++)
                    {
                        ...
                        for (int i = 0; i < before_data.Count; i++)
                        {
                            ...
                            try
                            {
                                double delta_R = Math.Abs(Convert.ToDouble(cycle_rgn.Offset(i, 0).Value) - Convert.ToDouble(cycle_rgn.Offset(i, 1).Value)) / Convert.ToDouble(cycle_rgn.Offset(i, 0).Value);
                                if (delta_R > 0.1)
                                {
                                    cycle_rgn.Offset(i, 2).Style.Fill.PatternType = ExcelFillStyle.Solid;
                                    cycle_rgn.Offset(i, 2).Style.Fill.BackgroundColor.SetColor(Color.Red);
                    ...
                    try
                    {
                        ProcessStartInfo pi = new ProcessStartInfo(daily_report_name);
                        Process.Start(pi);
                    }
                    catch
                    {
        ...
        public void Export_Thermal_HeatSoak_Bend_MultiType(SqlConnection sqlcon_OK2SHIP, string ItemCode, string LotNo, string process_name, string format_name, int tar_pcs, string format_type, string shift_no)
        {
            string format_loc = Path.Combine(find_config_path(Application.StartupPath, "SEEV Data"), "Format", format_type);
            Report_location = Path.Combine(find_config_path(Application.StartupPath, "SEEV Data"), "Report");
            ...
            if (format_file != "")
            {
                ...
                if (tar_wrksht_name != "")
                {
                    if (tar_pcs <= src_tbl_lst.Count)
                    {
                        ...
                        //string NET_start_rgn=Excel_Lib.Find_Cell_Addr("Test item","A10", tar_wrksht, false);
                        //int r_qty = Excel_Lib.Get_Cells_Info(tar_wrksht, tar_wrksht.Cells[NET_start_rgn]).row_qty;
                        ...
                        foreach (var item in data_lst)
                        {
                            ...
                            if (Echeck_Cycle_addr.Keys.ToList().IndexOf(item.Key) != -1)
                            {
                                ...
                                {
                                    sum_rgn_offset.Value = "FAIL";
                                    sum_rgn_offset.Offset(1, 0).Value = "FAIL";
                                }
                            }
                            if (item.Key.Contains("After"))
                            {
                                ...
                                if (cycle_data_addr_lst.Keys.ToList().IndexOf(cycle_no) != -1)
                                {
                                    ...
                                    for (int r_inx = 0; r_inx < sel_data.Count; r_inx++)
                                    {
                                        ...
                                        try
                                        {
                                            double delta_R = Math.Abs(Convert.ToDouble(before_data[r_inx]) - Convert.ToDouble(sel_data[r_inx])) / Convert.ToDouble(before_data[r_inx]);
                                            if (delta_R > 0.1)
                                            {
                        ...
                        try
                        {
                            ProcessStartInfo pi = new ProcessStartInfo(daily_report);
                            Process.Start(pi);
                        }
                        catch
                        {

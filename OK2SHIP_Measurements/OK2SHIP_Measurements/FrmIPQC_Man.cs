...
namespace OK2SHIP_Measurements
{
    public partial class FrmIPQC_Man : Form
    {
                ...
                if (init_data3())
                {
                    ...
                    if (dt_result.Rows.Count == 0)
                    {
                        foreach (DataGridViewColumn t in DGV_SpecView.Columns)
                        {
                            if(!myCode.check_columns_existed(myCode.DGV_To_Table(sel_DGV),t.Name))
                            {
                                ...
                                //else
                                //{
                                //    string setval = DGV_SpecView.Rows[0].Cells[t.Name].Value.ToString();
                                //    if (setval != "NA")
                                //    {
                                //        sel_DGV.Columns.Add(t.Name, t.HeaderText);
                                //    }
                                //}    
                                string setval = myCode.checkDBNull(DGV_SpecView.Rows[0].Cells[t.Name].Value);
                                if (setval != "NA")
                                {
                                    sel_DGV.Columns.Add(t.Name, t.HeaderText);
                                }
                    ...
                    else
                    {
                        if(sel_DGV.Rows.Count==0)
                        {
                            ...
                            List<string> adding_cols_lst = new List<string>();
                            List<string> db_cols_lst = new List<string>();
                            int inx = 0;
                            foreach (DataGridViewColumn t in DGV_SpecView.Columns)
                            {
                                if (TDMK_Code.check_exist_list_index2(t.Name, cur_col_arr) > -1)
                                {
                                    db_cols_lst.Add(t.Name);
                                }
                                else
                                {
                                    adding_cols_lst.Add(t.Name);
                                }
                            ...
                            DataTable dest_tbl = dt_result.AsDataView().ToTable(false, db_cols_lst.ToArray());
                            foreach(string added_col in adding_cols_lst)
                            {
                                dest_tbl.Columns.Add(added_col);
                            }
                            sel_DGV.DataSource = dest_tbl;
                            inx = 0;
                            foreach (DataGridViewColumn t in sel_DGV.Columns)
                            {
        ...
        {
            bool _result = false;
            DataTable cur_spec_dt = TDMK_Code.Datatable_Filter(sqlcon, "SpecList", TDMK_Code.filter_str(new string[] { "ItemCode", "Remark" }, new string[] { txtItemCode.Text, sel_tbl }));
            if (DGV_SpecView.Columns.Count>0)
            {
                ...
                else
                {
                    ...
                    List<string> spec_col_lst = cur_spec_dt.AsEnumerable().Select(x => x.Field<string>("Col_Name")).ToList();
                    for (int col_inx = 0;col_inx < DGV_SpecView.Columns.Count;col_inx++)
                    {
                        string col_name = DGV_SpecView.Columns[col_inx].Name;
                        if(spec_col_lst.IndexOf(col_name)!=-1)
                        {
                            string cur_spec_sel = cur_spec_dt.AsEnumerable().Where(x => x.Field<string>("Col_Name") == col_name).Select(x => x.Field<string>("Spec")).ToList().FirstOrDefault();
                            Spec_detail cur_spec = Get_Spec_detail(cur_spec_sel);
                            DGV_SpecView.Rows[0].Cells[col_name].Value = cur_spec.SetVal;
                            DGV_SpecView.Rows[1].Cells[col_name].Value = cur_spec.UL;
                            DGV_SpecView.Rows[2].Cells[col_name].Value = cur_spec.LL;
                        }
                    }
        ...
        public Spec_detail Get_Spec_detail(string _limit)
        {
            Spec_detail result =new Spec_detail();
            char[] split_chr = { '~', '<', '\u2264', '\u2265', '\u00B1', '>' };
            ...
            if (_limit.Contains('/'))
            {
                ...
                result.SetVal = sv.ToString();
                result.UL = (sv + ul).ToString();
                result.LL = (sv - ll).ToString();
            }
            else
            {
                ...
                if (_limit.Contains('>'))
                {
                    result.SetVal = _limit;
                    result.UL = UL_LL[0].Trim(trim_chr);
                    result.LL = UL_LL[1].Trim(trim_chr);
                }
                else
                {
                    if (_limit.Contains('\u00B1'))
                    {
                        result.SetVal = UL_LL[0];
                        result.UL = (Convert.ToDouble(UL_LL[0].Trim(trim_chr)) + Convert.ToDouble(UL_LL[1].Trim(trim_chr))).ToString();
                        result.LL = (Convert.ToDouble(UL_LL[0].Trim(trim_chr)) - Convert.ToDouble(UL_LL[1].Trim(trim_chr))).ToString();
                    }
                    else
                    {
                        if (_limit != "NA")
                        {
                            result.SetVal = _limit;
                            result.LL = UL_LL[0].Trim(trim_chr);
                            result.UL = UL_LL[1].Trim(trim_chr);
                        }
                        else
                        {
                            result.SetVal = _limit;
        ...
        //                            testFAI_spec[spec_col_inx] = new SEI_Lib.FAI_Spec(t_FAIName, t_checkside, t_FAI_Setval, t_FAI_UL, t_FAI_LL);
        ...
        //{
        //    SEI_Lib. FAI_Spec sel_FAI_test = testFAI_spec[0];
        ...
        //        txtOperator.Text = SEI_Lib.curr_user;
        //    }
        //    else
        //    {
        //        SEI_Lib.curr_user = txtOperator.Text;
        ...
        {
            List<string> dest_col_lst = new List<string>();
            DataTable cur_spec_dt = TDMK_Code.Datatable_Filter(sqlcon, "SpecList", TDMK_Code.filter_str(new string[] { "ItemCode", "Remark" }, new string[] { txtItemCode.Text, sel_tbl }));
            dest_col_lst = cur_spec_dt.AsEnumerable().Select(x => x.Field<string>("Col_Name")).ToList();
            List<string> DGV_Spec_col_lst = new List<string>();
            foreach(DataGridViewColumn dgv_c in DGV_SpecView.Columns)
            {
                DGV_Spec_col_lst.Add(dgv_c.Name);
            }
            List<string> sel_spec_col_lst = dest_col_lst.Intersect(DGV_Spec_col_lst).ToList();
            if(sel_spec_col_lst.Count>0)
            {
                DataTable tar_spec_dt = myCode.DGV_To_Table(DGV_SpecView).AsDataView().ToTable(false, sel_spec_col_lst.ToArray());
                if (myCode.insert_IPQC_data_DGV(DGV_Data_Plus, DGV_DataView, tar_spec_dt))
                {
                    myCode.Check_Data_inSpec(DGV_DataView, DGV_SpecView);
                }
            }
        }
        ...
        private void chklstPart_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            if (!myCode.check_columns_existed(myCode.DGV_To_Table(DGV_SpecView), col_name))
            {
                string spec_val = sel_dt.AsEnumerable().Where(x => x.Field<string>("Col_Name") == col_name).Select(x => x.Field<string>("Spec")).ToList().FirstOrDefault();
                if(spec_val!="NA")
                {
                    DGV_SpecView.Columns.Add(col_name, col_header);
                }
                ...
                {
                    MessageBox.Show("Không thể chọn vì spec = NA", "Thông báo");
                }
        ...
        {
            myCode.Lotno_Formated(txtLotNo.Text);
        }
    }
}
